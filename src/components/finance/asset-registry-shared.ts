...
export function buildRegistryData(assets: Asset[], rates: PriceRate[] = []) {
	...
	for (const item of liquidItems) {
		(liquidGroups[item.typeLabel] = liquidGroups[item.typeLabel] || []).push(
			item,
		);
	}
	...
	for (const item of nonLiquidItems) {
		(nonLiquidGroups[item.typeLabel] =
			nonLiquidGroups[item.typeLabel] || []).push(item);
	}
	...
	const totalNilai =
		liquidItems.reduce((sum, item) => sum + item.value, 0) + nonLiquidValue;
