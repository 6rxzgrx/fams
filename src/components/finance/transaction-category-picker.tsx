...
export function TransactionCategoryPicker({
	...
	const activeType = allowedTypes.includes(manualType)
		? manualType
		: normalizedDefaultType;

	const typeCategories = searchSelectableCategories(
		categories,
		deferredQuery,
		activeType,
	);
	const favoriteCategories = typeCategories.filter((c) =>
		favoriteIds.includes(c.id),
	);
	const fullListCategories = typeCategories.filter(
	...
	function handleOpenChange(nextOpen: boolean) {
		...
		if (nextOpen) {
			...
			const syncedType =
				cat && allowedTypes.includes(cat.type)
					? cat.type
					: normalizedDefaultType;
				...
				<div className="min-w-0 flex-1">
					<p
						className={cn(
							...
							!selectedCategory && 'font-normal',
						)}
					>
						{selectedCategory
							? formatCategoryLabel(selectedCategory, categories)
							: placeholder}
					</p>
					<p className="mt-0.5 text-xs text-muted-foreground">
						{selectedCategory
							? CATEGORY_TYPE_LABELS[selectedCategory.type]
							: label}
					</p>
				</div>
				<Star
					className={cn(
						'size-4 shrink-0',
						selectedCategory && isFavorite(selectedCategory.id)
				...
				<DialogContent className="gap-0 overflow-hidden p-0 sm:max-w-lg">
					...
					<div className="border-b border-border px-5 py-3">
						{allowedTypes.length > 1 && (
							<div className="mb-3 grid grid-cols-2 gap-1.5 rounded-xl bg-muted/60 p-1">
								...
								{hasVisibleBranches ? (
									<div className="space-y-2">
										{branches.map((branch) => {
											...
											const isStandaloneFavorite =
												!hasChildren && isFavorite(branch.parent.id);
											...
											if (!hasChildren) {
												return (
													<FlatCategoryRow
														...
														onToggleFavorite={() =>
															toggleFavorite(branch.parent.id)
														}
													/>
												);
											}
																...
																return (
																	...
																	>
																		...
																		<button
																			...
																			aria-label={
																				isFavorite(child.id)
																					? `Hapus ${child.name} dari favorit`
																					: `Tambahkan ${child.name} ke favorit`
																			}
																		>
																			<Star
																				className={cn(
