...
import { ChevronDown, Plus, Tags, Pencil, Sparkles, Trash2 } from 'lucide-react'
...
import type { CategoryType, CreateTransactionCategoryInput, TransactionCategory } from '@/domain/types'
...
export function FinanceCategorySettings() {
  ...
  const [createDefaults, setCreateDefaults] = useState<Partial<TransactionCategory> | null>(null)
  ...
  return (
    <div className="space-y-4">
      <header className="flex items-start justify-between gap-3">
        <div>
          <h2 className="text-[20px] font-semibold leading-tight tracking-tight lg:text-[24px]">Kategori</h2>
          <p className="mt-1 text-[13px] text-muted-foreground">
            Atur kategori transaksi keluarga dengan struktur ringan: kategori mandiri atau induk dengan satu level anak.
          </p>
        </div>
        <Button
          variant="accent"
        ...
        <EmptyState
          ...
          action={
            <div className="flex flex-col items-center gap-2 sm:flex-row">
              {categories.length === 0 && (
                <Button variant="accent" onClick={handleSeed} disabled={seeding} aria-busy={seeding}>
                  <Sparkles className="size-4" strokeWidth={2.25} aria-hidden="true" />
                  {seeding ? 'Memuat...' : 'Muat Kategori Default'}
                </Button>
              )}
              <Button variant="outline" onClick={() => setCreateDefaults({ type: activeType, parent_id: '' })}>
                <Plus className="size-4" strokeWidth={2.5} aria-hidden="true" />
                Buat Manual
              </Button>
            </div>
      ...
      {!isLoading && !error && branches.length > 0 && (
        <div className="space-y-3">
          {branches.map((branch) => {
            ...
            return (
              <section key={branch.parent.id} className="overflow-hidden rounded-xl border border-border bg-surface">
                {/* Parent header */}
                <div className={cn(
                  ...
                  {hasChildren ? (
                    <button
                      ...
                      aria-label={isCollapsed ? 'Buka kategori anak' : 'Tutup kategori anak'}
                  ...
                  {/* Color dot */}
                  <span
                    className="inline-flex size-9 shrink-0 rounded-pill border border-border"
                  ...
                  <div className="min-w-0 flex-1">
                    <p className="truncate font-medium">{branch.parent.name}</p>
                    <p className="truncate text-xs text-muted-foreground">
                      {hasChildren ? `${branch.children.length} kategori anak` : 'Bisa dipakai langsung di transaksi'}
                    </p>
                  </div>
                ...
                {hasChildren && !isCollapsed && (
                  <div className="divide-y divide-border">
                    {branch.children.map((category) => (
                      ...
                      >
                        <span
                          className="inline-flex size-7 shrink-0 rounded-full border border-border/60"
                        ...
                        <Pencil className="size-4 shrink-0 text-muted-foreground" strokeWidth={2} aria-hidden="true" />
                      </button>
                    ))}
                  </div>
                )}
      ...
      <Dialog open={!!createDefaults} onOpenChange={(open) => !open && setCreateDefaults(null)}>
        <DialogContent>
          <DialogHeader>
          ...
          {createDefaults && (
            <CategoryForm
              ...
              loading={creating}
              forceType={createDefaults.parent_id ? (createDefaults.type as CategoryType) : undefined}
            />
          )}
        </DialogContent>
      </Dialog>
        ...
        <DialogContent>
          ...
          {editCategory && (
            <div className="space-y-4">
              ...
              <Button
                variant="destructive"
                className={cn('w-full', deleting && 'opacity-70')}
